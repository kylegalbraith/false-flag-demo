...
- **"How this is different from alternatives"** -> Depot CI runs your _uncommitted local diff_ via API/CLI in seconds. Every other CI (GHA, CircleCI, Buildkite) is push-triggered and a black box. This is "push-wait-guess" vs. a local closed loop.

The one-line story (already drafted in [.demo/DEMO.md](.demo/DEMO.md)): _an agent validates its own uncommitted code against real CI in seconds, reads the failure programmatically, fixes it, reruns only the failed job, and pushes once green, having never pushed a broken commit._
...
To keep a _live_ demo reliable, pre-rehearse one specific change that produces a clean, self-inflicted failure the agent can diagnose and fix:
...
2. **Ensure the snapshot image is fresh**: trigger [.depot/workflows/snapshot-e2e.yml](.depot/workflows/snapshot-e2e.yml) once so the warm image exists in the registry (it is `workflow_dispatch`). This is also a great thing to _show_ as "the agent baked its dependencies once."
...
In Cursor, prompt the agent: _"Add a `starts_with` string operator to the flag targeting engine and a corpus fixture that uses it."_ It edits [internal/eval/predicates.go](internal/eval/predicates.go) (+ config wiring + a fixture). Leave it **uncommitted**. "No commit. No push. This is just my working tree."
...
"It uploaded my _uncommitted diff_ as a patch and ran it on real CI. No other CI on the market does this." It fails (Go has the operator, TS twin doesn't). Agent reads it programmatically:
...
Green. "It reran _only_ the job that mattered - intelligence about which validation loop to run - and it took seconds because the sandbox was warm."
...
- API/CLI-driven, runs _uncommitted local diffs_ -> the only CI that fits inside an agent's inner loop.
...
- Everyone else is push-triggered and opaque. As agents write more code, the bottleneck moves from writing to _verifying_ - and Depot is the verification substrate that makes agentic engineering safe and fast.
